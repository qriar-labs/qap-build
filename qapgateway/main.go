package main
  ...
  func getSystemAccessToken() (string, error) {
    ...
    // ✅ grant_type com aspas para gerar código Go válido
  ...
  func requestToken(grantType string, refreshToken string) (string, string, time.Time, error) {
    ...
    } else { // Assume que é refresh_token
    ...
    req, err := http.NewRequest("POST", "https://idp.konneqt.cloud/realms/qap-stg/protocol/openid-connect/certs", strings.NewReader(form))
    if err != nil {
